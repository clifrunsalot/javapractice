...
/**
 * This instantiates the Triathlon Electronic Workout Log application. It offers
 * five features:<br>
 * <br>
 * &nbsp;&nbsp;&nbsp;- Electronic logging of daily workouts involving running,
 * swimming, and biking;<br>
 * &nbsp;&nbsp;&nbsp;- Graphing capability for any of the three activities over
 * time from first to last entry;<br>
 * &nbsp;&nbsp;&nbsp;- Electronic maintenance logging of exercise equipment
 * (i.e. - bike);<br>
 * &nbsp;&nbsp;&nbsp;- Maintains specifications on exercise equipment and attire
 * used for running, swimming, and biking;<br>
 * &nbsp;&nbsp;&nbsp;- A distance-time-pace conversion calculator.<br>
 * <br>
 * All information used by the program is stored in a single file named by the
 * user and which must end with the file extension ".dat."
 */
public class Tewl extends JFrame implements ActionListener, ChangeListener {
	/*
	 * Constructor: Tewl
	 */
	public Tewl() {
		...
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				...
				if (!identical(origlogbook, logbook) || !identical(origmntlogbook, mntlogbook)
						|| !identical(origequiplogbook, equiplogbook) || !identical(origoptionslogbook, optionslogbook)
								&& !origlogbook.isEmpty() && !logbook.isEmpty()) {
					int option = JOptionPane.showConfirmDialog(contentPane, "Do you want to save your latest changes?",
							"Save Latest Changes?", JOptionPane.YES_NO_OPTION);
	...
	/**
	 * Fills a JComboBox with numbers beginning with "start" and ending with
	 * "last"
	 * 
	 * @param comboBox
	 *            JComboBox component to fill
	 * @param start
	 ...
	 *            JComboBox component to fill
	 * @param strArray
	 *            Array of strings used to fill comboBox
	 * 
	 * @return void
	 */
	 ...
	 * 
	 * @param optionslogbook
	 *            Vector containing list of options
	 * @param optionName
	 *            String name for set of options to extract from vector
	 * 
	...
	public void makeActionPanel() {
		...
		Component[] nab = { findButton, newButton, deleteButton, editButton, saveButton, cancelButton, clearButton };
	...
	public String getTodaysDate() {
		...
		return (weekDay + " - " + monthStr + " " + caldate + ", " + calyear);
	 ...
	 * 
	 * @param gbc
	 *            GridBagConstraints to establish layout
	 * 
	 * @return void
	 */
	public void makeAerobicPanel(GridBagConstraints gbc) {
		...
		scrollPane = new JScrollPane(summaryTextArea, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		JTextField[] tf = { monthField, dayField, yearField, statusField, puField, suField };// ~JTextField
																								// []
																								// tf
																								// =...
	...
	public void makeStrengthPanel(GridBagConstraints gbc) {
		...
		puField.setHorizontalAlignment(JTextField.RIGHT);
		...
		suField.setHorizontalAlignment(JTextField.RIGHT);
	...
	public void makeMaintenancePanel(GridBagConstraints gbc) {
		...
		maintDateField.setHorizontalAlignment(JTextField.RIGHT);
		...
		scrollPane = new JScrollPane(maintWorkTextArea, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	public void makeNavigationButtonPanel() {
		...
		JButton[] nab = { beginButton, prevButton, nextButton, endButton };
	...
	public void makeStatusPanel() {
		...
		todaysDateField.setHorizontalAlignment(JTextField.CENTER);
	...
	public void makeStatisticsPanel(GridBagConstraints gbc) {
		...
		totalMilesToDateField.setHorizontalAlignment(JTextField.RIGHT);
		...
		avgMilesField.setHorizontalAlignment(JTextField.RIGHT);
		...
		statsAvgSpeedField.setHorizontalAlignment(JTextField.RIGHT);
	...
	public void makeCalculatorPanel(GridBagConstraints gbc) {
		...
		calcDistanceMilesField.setHorizontalAlignment(JTextField.RIGHT);
		...
		calcPaceMphField.setHorizontalAlignment(JTextField.RIGHT);
		...
		calcPaceMinPerMileField.setHorizontalAlignment(JTextField.RIGHT);
		...
		calcDistanceKiloField.setHorizontalAlignment(JTextField.RIGHT);
		...
		calcPaceMinPer100ydsField.setHorizontalAlignment(JTextField.RIGHT);
		...
		calcPaceKphField.setHorizontalAlignment(JTextField.RIGHT);
		...
		calcTimeField.setHorizontalAlignment(JTextField.RIGHT);
		...
		calcPaceMinPerKmField.setHorizontalAlignment(JTextField.RIGHT);
		...
		calcPaceMinPer100mField.setHorizontalAlignment(JTextField.RIGHT);
	...
	public void showAboutDialog() {
		...
		about.setFont(f);
		about.showConfirmDialog(this,
				"T.E.W.L.\n" + "Triathlon Electronic Workout Log\n" + "Copyright 2002 by Clif Hudson\n"
						+ "Version: 1.0\n" + "Date: September 4, 2002",
				"About T.E.W.L.", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null);
	 ...
	 * 
	 * @param c
	 *            Panel component to add to panel
	 * @param gbc
	 *            GridBagConstraints to establish layout
	 * 
	 * @result void
	 */
	public void add(JPanel p, Component c, GridBagConstraints gbc, int x, int y, int w, int h) {
		gbc.gridx = x;
		gbc.gridy = y;
		gbc.gridwidth = w;
		gbc.gridheight = h;
	...
	/**
	 * Compares date from query to each one in list and, if applicable, returns
	 * the index of the matching record. Otherwise, it returns -1, which means
	 * the record was not found.
	 * 
	 * @param s
	 *            An array of Strings representing the Month, Day, and Year
	 * 
	...
	public int recordFound(String[] s) {
		...
		while (iter.hasNext()) {
			...
			if ((m == w.getDate().getMonth()) && (d == w.getDate().getDay()) && (y == w.getDate().getYear())) {
				break;
			} else
				currentIndex = -1;
		} // ~while (iter.hasNext())...
	...
	/**
	 * Resets all text components to blank and radio buttons to bike (event) and
	 * mild (weather).
	 * 
	 * @param none
	 * 
	...
	public int getCurrentWindowIndex() {
		...
		while (iter.hasNext()) {
			...
			if ((w.getDate().getMonth() == cmonth) && (w.getDate().getDay() == cday)
	...
	/**
	 * Validates all field info, and if all correct, returns filled Workout
	 * object.
	 * 
	 * @param none
	 * 
	...
	public Workout getFields() {
		...
		boolean validMonth = false, validDay = false, validYear = false, validTemp = false, validWeather = false,
				validPulse = false, validWeight = false, validEvents = false, validPu = false, validSu = false;
		...
		if (validMonth && validDay && validYear && validWeather && validTemp && validPulse && validWeight && validEvents
				&& validSu && validPu) {
			// set month, day, year
			...
			recordToSave.setEvent("run", we[0].getDistance(), we[0].getTime().getTime(), we[0].getRoute().getCode());

			recordToSave.setEvent("bike", we[1].getDistance(), we[1].getTime().getTime(), we[1].getRoute().getCode());

			recordToSave.setEvent("swim", we[2].getDistance(), we[2].getTime().getTime(), we[2].getRoute().getCode());
	...
	/**
	 * Populates window with workout object passed in and makes the record
	 * uneditable.
	 * 
	 * @param w
	 *            Workout object to populate Aerobic form.
	 * 
	 * @return void
	 */
	public void populateWindow(Workout w) {
		...
		statusField.setText("Workout Record " + (logbook.indexOf(w) + 1) + " of " + logbook.size());

		// set variables for use inside statusField
		visibleWorkoutRecord = logbook.indexOf(w) + 1;
		logbooksize = logbook.size();

		JButton[] ba3 = { findButton, editButton, cancelButton, newButton, saveButton, deleteButton, clearButton,
				beginButton, prevButton, nextButton, endButton };
		...
		if (!logbook.isEmpty()) {
			...
			if (findDialog.isValidRequest()) {
				...
				if (recordIndex >= 0)
					...
					errorDialog.showMessageDialog(this, "No record found", "Error", JOptionPane.ERROR_MESSAGE);
				}
			...
			JOptionPane errorDialog = new JOptionPane();
			errorDialog.showMessageDialog(this,
					"To use this feature, there must be more\n" + "than one record in the database.", "Error",
	...
	public void edit() {
		...
		if (!logbook.isEmpty()) {
			...
			// and clear.
			JButton[] ba = { findButton, editButton, newButton, beginButton, prevButton, nextButton, endButton };

			componentsEnabled(ba, false);
		} // ~if(!logbook.isEmpty())...

		else {
			JOptionPane.showMessageDialog(this, "A new record must be created\n" + "before it can be edited.", "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}// ~public void edit()...
			...
			if (tempComboBox.isEnabled() && puField.isEditable()) {
				if (monthField.getText().trim().length() > 0 && dayField.getText().trim().length() > 0
					...
					if (cindex != -1) {
						...
						populateWindow((Workout) logbook.elementAt(cindex));
					}
					...
					// disabled previously
					Component[] compOn = { findButton, editButton, cancelButton, newButton, saveButton, deleteButton,
							clearButton, beginButton, prevButton, nextButton, endButton };// ~Component
																							// []
																							// compOn
																							// =...
				...
				JOptionPane.showMessageDialog(this, "This record must be in the Edit mode first", "Error",
						JOptionPane.ERROR_MESSAGE);
		} // ~if (!logbook.isEmpty())...
		...
		if (tempComboBox.isEnabled() && puField.isEditable()) {
			...
			if (newRecordValid) {
				...
				if ((index >= 0) && (!logbook.isEmpty())) {
					...
					int answer = JOptionPane.showConfirmDialog(this, "Do you want to save/modify this existing record?",
	...
	public void delete() {
		...
		if (tempComboBox.isEnabled() && puField.isEditable()) {
			int answer = JOptionPane.showConfirmDialog(this, "Delete record", "Confirmation", JOptionPane.YES_NO_OPTION,
	...
	public void clear() {
		...
		// If all field are not editable, then ask operator to place state in
		// Edit mode.
		...
		// date.
			...
			summaryTextArea.setText("");
			Component[] compOff = { findButton, editButton, deleteButton, newButton, beginButton, prevButton,
					nextButton, endButton };// ~Component [] compOff =...
	...
	public void getFirst() {
		...
		if (!logbook.isEmpty())
			...
			JOptionPane.showMessageDialog(this, "There are no records in the log.", "Error", JOptionPane.ERROR_MESSAGE);
	}
	...
	public void getPrev() {
		...
		if (!logbook.isEmpty()) {
			...
			if (iter.hasPrevious())
				...
				JOptionPane errorDialog = new JOptionPane();
				errorDialog.showMessageDialog(this, "There are no more previous records to view", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		} // ~if(!logbook.isEmpty())...
		...
		if (!logbook.isEmpty()) {
			...
			if (iter.hasNext()) {
				...
				if (iter.hasNext())
					...
					JOptionPane.showMessageDialog(this, "There are no more records to view", "Error",
							JOptionPane.ERROR_MESSAGE);
				}
			} // ~if (iter.hasNext())...
	...
	public void populateMaintenanceTab(Maintenance m) {
		...
		JButton[] ba3 = { editButton, cancelButton, newButton, saveButton, deleteButton, clearButton, prevButton,
				nextButton };
	...
	public void getPrevMaintRecord() {
		...
		if (pindex != -1) {
			...
			if (piter.hasPrevious())
				...
				JOptionPane.showMessageDialog(this, "There are no previous records to view.", "Error",
						JOptionPane.ERROR_MESSAGE);
		} // ~if(pindex != -1)...
	...
	public void editMaintRecord() {
		...
		JButton[] ba = { deleteButton, findButton, cancelButton, clearButton };

		componentsEnabled(ba, true);
		JButton[] ba2 = { editButton, newButton, prevButton, nextButton };
	...
	public void cancelMaintRecord() {
		...
		if (maintWorkTextArea.isEditable()) {
			populateMaintenanceTab((Maintenance) mntlogbook.lastElement());
			maintDateField.setEditable(false);
			maintWorkTextArea.setEditable(false);
			// Enable all button in case they were
			// disabled previously
			JButton[] ba1 = { editButton, cancelButton, newButton, saveButton, deleteButton, clearButton, prevButton,
					nextButton };
		...
		if (maintWorkTextArea.isEditable()) {
			...
			if ((index >= 0) && (!mntlogbook.isEmpty())) {
				...
				if (answer == JOptionPane.YES_OPTION) {
					...
					// Reload original, unmodified record
					populateMaintenanceTab((Maintenance) mntlogbook.elementAt(index));
			} // ~if((index >= 0) && (!mnt...
	...
	public void deleteMaintRecord() {
		...
		if (maintWorkTextArea.isEditable()) {
			...
			if (answer == JOptionPane.YES_OPTION) {
				JButton[] ba = { findButton, saveButton, editButton, newButton, cancelButton, clearButton };

				componentsEnabled(ba, true);
				JButton[] ba2 = { deleteButton };
	 ...
	 *            Array of Components
	 * @param b
	 *            True to make components enabled or False to make components
	 *            unenabled.
	 * 
	 * @return void
	 */
	 ...
	 *            Array of components
	 * @param b
	 *            True to make components editable or False to make them
	 *            uneditable
	 * 
	 * @return void
	 */
	 ...
	 * 
	 * @param event
	 *            Name of event to use to populate Statistics Form
	 * 
	 * @return void
	 */
	...
	public void calcFields() {
		...
		TewlUtilities util = new TewlUtilities();
		double factor1, factor2, result, convertToKiloDist, convertToKiloPace, convertToMileDist, convertToMilePace,
				mins, secs = 0.0;
		...
		// miles -> time -> mph
		// Time = Miles/(mph*60)
		if (calcMilesCheck.isSelected() && calcTimeRadio.isSelected() && calcPaceMphRadio.isSelected()) {
			if (DEBUG) {
				d.print("1");
			}
			if (util.validDouble(calcPaceMphField.getText().trim(), 6)
				...
				calcPaceMinPer100ydsField.setText(t.formatTime((60 * 100) / (factor1 * 1760)));

				// Min Per Km
				calcPaceMinPerKmField.setText(t.formatTime(60 / convertToKiloPace));

				// mins per 100m
				// mph to mins/100m factor = (0.625 miles * 60 mins * 100 m) /
				// 1000 m
				calcPaceMinPer100mField.setText(t.formatTime((0.625 * 60 * 100) / (factor1 * 1000)));

				String sel = calcSelectionGroup.getSelection().getActionCommand();
			} // ~util.validDouble(calcDistance...

			else {
				JOptionPane.showMessageDialog(this, "Please correct all invalid entries.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		...
		// Time = (mins / mile) * (Distance in miles)
		else if (calcMilesCheck.isSelected() && calcTimeRadio.isSelected() && calcPaceMinPerMileRadio.isSelected()) {
			if (DEBUG) {
				d.print("2");
			}
			if (t.validTime(calcPaceMinPerMileField.getText().trim())
				...
				calcPaceMinPer100ydsField.setText(t.formatTime((factor1 * 100) / 1760));

				// Min Per Km
				calcPaceMinPerKmField.setText(t.formatTime(60 / convertToKiloPace));

				// mins per 100m
				// minpermile to mins/100m factor = (0.625 miles * 100 m) / 1000
				// m
				calcPaceMinPer100mField.setText(t.formatTime((factor1 * 0.625 * 100) / 1000));
		...
		//
		else if (calcMilesCheck.isSelected() && calcTimeRadio.isSelected() && calcPaceMinsPer100ydsRadio.isSelected()) {
			if (DEBUG) {
				d.print("3");
			}
			if (util.validDouble(calcDistanceMilesField.getText().trim(), 6)
				...
				calcPaceMinPerKmField.setText(t.formatTime((factor2 * 1760 * 0.625) / 100));

				// mins per 100m
				// minpermile to mins/100m factor = (1760yds * .625mi *
				// 100m)/1000m
				calcPaceMinPer100mField.setText(t.formatTime((factor2 * 1760 * 0.625) / 1000));
		...
		// miles -> distance -> mph
		// miles = (mph*60)/Time
		else if (calcMilesCheck.isSelected() && calcDistanceRadio.isSelected() && calcPaceMphRadio.isSelected()) {
			if (DEBUG) {
				d.print("4");
			}
			if (util.validDouble(calcPaceMphField.getText().trim(), 6) && t.validTime(calcTimeField.getText().trim())) {
				// Pace(mph)
				factor1 = Double.parseDouble(calcPaceMphField.getText().trim());
		...
		else if (calcMilesCheck.isSelected() && calcDistanceRadio.isSelected()
				&& calcPaceMinPerMileRadio.isSelected()) {
			if (DEBUG) {
				d.print("5");
			}
			if (t.validTime(calcTimeField.getText().trim()) && t.validTime(calcPaceMinPerMileField.getText().trim())) {
				// Time
		...
		//
			...
			if (t.validTime(calcTimeField.getText().trim())
				...
				// min per mile
				calcPaceMinPerMileField.setText(t.formatTime((factor2 * 1760) / 100));
		...
		// Pace = Miles/(Time/60)
		else if (calcMilesCheck.isSelected() && calcPaceRadio.isSelected()) {
			if (DEBUG) {
				d.print("7");
			}
			if (util.validDouble(calcDistanceMilesField.getText().trim(), 6)
				...
				// min per 100yds
				calcPaceMinPer100ydsField.setText(t.formatTime((factor2 * 100) / (factor1 * 1760)));

				// min per 100m
				calcPaceMinPer100mField.setText(t.formatTime((factor2 * .625 * 100) / (factor1 * 1000)));
		...
		// Time = Kilos/((distance in km)/60)
		else if (calcKilosCheck.isSelected() && calcTimeRadio.isSelected() && calcPaceKphRadio.isSelected()) {
			if (DEBUG) {
				d.print("8");
			}
			if (util.validDouble(calcPaceKphField.getText().trim(), 6)
				...
				// min per 100yds
				calcPaceMinPer100ydsField.setText(t.formatTime((100 * 60) / (factor1 * .625 * 1760)));

				// min per 100m
				calcPaceMinPer100mField.setText(t.formatTime((100 * 60) / (factor1 * 1000)));
		...
		// kilos -> time -> min/km
		else if (calcKilosCheck.isSelected() && calcTimeRadio.isSelected() && calcPaceMinPerKmRadio.isSelected()) {
			if (DEBUG) {
				d.print("9");
			}
			if (t.validTime(calcPaceMinPerKmField.getText().trim())
				...
				// min per 100yds
				calcPaceMinPer100ydsField.setText(t.formatTime((factor1 * 100) / (.625 * 1760)));

				// min per 100m
				calcPaceMinPer100mField.setText(t.formatTime((factor1 * 100) / 1000));
		...
		// kilos -> time -> mins/100m
		else if (calcKilosCheck.isSelected() && calcTimeRadio.isSelected() && calcPaceMinsPer100mRadio.isSelected()) {
			if (DEBUG) {
				d.print("10");
			}
			...
			if (util.validDouble(calcDistanceKiloField.getText().trim(), 6)
				...
				// time
				calcTimeField.setText(t.formatTime((factor1 * factor2 * 1000) / 100));
				...
				// Min Per Mile
				calcPaceMinPerMileField.setText(t.formatTime((factor2 * 1000) / (100 * .625)));

				// Min Per Km
				calcPaceMinPerKmField.setText(t.formatTime((factor2 * 1000) / 100));

				// min per 100yds
				calcPaceMinPer100ydsField.setText(t.formatTime((factor2 * 1000 * 100) / (100 * .625 * 1760)));
		...
		else if (calcKilosCheck.isSelected() && calcDistanceRadio.isSelected() && calcPaceKphRadio.isSelected()) {
			if (DEBUG) {
				d.print("11");
			}
			if (util.validDouble(calcPaceKphField.getText().trim(), 6) && t.validTime(calcTimeField.getText().trim())) {
				// Pace (kph)
				factor1 = Double.parseDouble(calcPaceKphField.getText().trim());
				...
				// min per 100yds
				calcPaceMinPer100ydsField.setText(t.formatTime((60 * 100) / (factor1 * .625 * 1760)));
		...
		else if (calcKilosCheck.isSelected() && calcDistanceRadio.isSelected() && calcPaceMinPerKmRadio.isSelected()) {
			if (DEBUG) {
				d.print("12");
			}

			if (t.validTime(calcTimeField.getText().trim()) && t.validTime(calcPaceMinPerKmField.getText().trim())) {
				// Time
				factor1 = t.getConvertedTime(calcTimeField.getText().trim());
				...
				// min per 100yds
				calcPaceMinPer100ydsField.setText(t.formatTime((factor2 * 100) / (.625 * 1760)));

				// min per 100m
				calcPaceMinPer100mField.setText(t.formatTime((factor2 * 100) / 1000));
		...
		else if (calcKilosCheck.isSelected() && calcDistanceRadio.isSelected()
				&& calcPaceMinsPer100mRadio.isSelected()) {
			if (DEBUG) {
				d.print("13");
			}

			if (t.validTime(calcTimeField.getText().trim()) && t.validTime(calcPaceMinPer100mField.getText().trim())) {
				// Time
				factor1 = t.getConvertedTime(calcTimeField.getText().trim());
				...
				// Pace (mph)
				resultStr = formatter.format((100 * 60 * .625) / (factor2 * 1000));
				calcPaceMphField.setText(resultStr);
		...
		// kilometers -> pace
		else if (calcKilosCheck.isSelected() && calcPaceRadio.isSelected()) {
			if (DEBUG) {
				d.print("14");
			}
			if (util.validDouble(calcDistanceKiloField.getText().trim(), 6)
				...
				// Min Per 100yds
				calcPaceMinPer100ydsField.setText(t.formatTime((factor2 * 100) / (factor1 * .625 * 1760)));

				// Min Per 100m
				calcPaceMinPer100mField.setText(t.formatTime((factor2 * 100) / (factor1 * 1000)));
	 ...
	 * 
	 * @param tf
	 *            Array of JTextField components.
	 * @param b
	 *            Boolean value set to True to make fields editable or False to
	...
	public boolean hasValidFirstRecord() {
		...
		try {
			...
			if (w.getDate().getMonth() >= 0 && w.getDate().getDay() >= 0 && w.getDate().getYear() > 0
					&& w.getTemp() >= 0 && w.getWeather().length() != 0
					&& ((events[0].getName() != "" && events[0].getDistance() > 0
							&& events[0].getTime().toString() != "")
							|| (events[1].getName() != "" && events[1].getDistance() > 0
									&& events[1].getTime().toString() != "")
							|| (events[2].getName() != "" && events[2].getDistance() > 0
	...
	/**
	 * Determines whether or not the first record in the maintenance log is
	 * valid.
	 * 
	 * @param none
	 * 
	...
	public boolean hasValidFirstMaintRecord() {
		...
		if (m.getMaintDate().length() > 0 && m.getMaintSummary().length() > 0) {
			validRecord = true;
		}
	...
	public void editEquipPanel() {
		...
		if (selectedEvent.equals("equiprun")) {
			Component[] compOn = { equipRunShoeSizeField, equipRunShoeModelField };

			componentsEditable(compOn, true);
		} // ~if (selectedEvent.equals...

		else if (selectedEvent.equals("equipbike")) {
			Component[] compOn = { equipBikeNameField, equipBikeModelField, equipBikeSNField, equipBikeFrameSizeField,
					equipBikeTireSizeField, equipBikeShoeSizeField, equipBikeShoeModelField,
					equipBikeSeatPostDiameterField };// ~Component [] compOn
														// =...
			...
			Component[] compOn = { equipSwimGoggleSizeField };

			componentsEditable(compOn, true);
		}

		else if (selectedEvent.equals("equipwalk")) {
			Component[] compOn = { equipWalkShoeSizeField, equipWalkShoeModelField };
	 ...
	 * 
	 * @param event
	 *            Name of event to use to populate the Equipment form
	 * 
	 * @return void
	 */
	 ...
	 * 
	 * @param on
	 *            True to turn on buttons or False to turn off buttons
	 * 
	 * @return void
	 */
	public void navButtonsOn(boolean on) {
		...
		Component[] compOn = { beginButton, prevButton, nextButton, endButton };// ~Component
																				// []
																				// compOn
																				// =...
	 ...
	 * 
	 * @param on
	 *            True to turn buttons on or False to turn them off
	 * 
	 * @return void
	 */
	public void actionButtonsOn(boolean on) {
		...
		Component[] compOn = { findButton, newButton, deleteButton, editButton, editButton, saveButton, cancelButton,
				clearButton, };// ~Component [] compOn =...
	...
	public void resetForm() {
		...
		currentFileName = "";
		// enable
		Component[] compOn = { openItem, newItem, exitItem };

		componentsEnabled(compOn, true);
		// disable
		Component[] compOff = { printItem, saveItem, saveAsItem, backupItem, closeItem, removeallItem, optionsItem,
				findButton, newButton, deleteButton, editButton, saveButton, cancelButton, clearButton, beginButton,
				prevButton, nextButton, endButton };// ~Component [] compOff
													// =...

		componentsEnabled(compOff, false);

		// make all textfields uneditable
		Component[] tfOff = { monthField, dayField, yearField, statusField, puField, suField };// ~Component
																								// []
																								// tfOff
																								// =...
	...
	public boolean saveCurrentFile(Component parent) {
		...
		try {
			if (hasValidFirstRecord()) {
				...
				JOptionPane.showMessageDialog(this, "The database has no valid records.\n"
						+ "Please create a new database and save\n" + "a new record before saving the file.", "Error",
						JOptionPane.ERROR_MESSAGE);
			} // ~else...
		} // ~try...
		...
		try {
			if (!logbook.isEmpty() && hasValidFirstRecord()) {
				...
				if (result == JFileChooser.APPROVE_OPTION) {
					String str1 = fc.getCurrentDirectory() + System.getProperty("file.separator")
				...
				if (!fileSaved) {
					currentFileName = "";
					fileSaved = false;
					JOptionPane.showMessageDialog(this, "Please select a valid file name.", "Error",
							JOptionPane.ERROR_MESSAGE);
				} // ~else...
			} // ~if(!logbook.isEmpty() &&...
	...
	public void deleteCRs(String file) throws Exception {
		...
		try {
			...
			out.close();
			int j = 0;
			BufferedReader in2 = new BufferedReader(new FileReader("tmp.dat"));
	 ...
	 * }
	 * 
	 * public void printWorkout(Workout w, int record) { p = new
	 * PrintPanel(w,record); PrinterJob printJob = PrinterJob.getPrinterJob();
	 * PageFormat pageFormat = printJob.defaultPage();
	 * printJob.setPrintable(p,pageFormat); if(printJob.printDialog()) { try {
	 * printJob.print(); } catch(PrinterException exception) {
	 ...
	 *            Name of parent form
	 * 
	 * @return True if the file was loaded successfully or False if there was a
	 *         problem
	 */
	public boolean loadFile(Component parent) throws Exception {
		...
		try {
			...
			if (result == JFileChooser.APPROVE_OPTION) {
				currentFileName = fc.getCurrentDirectory() + System.getProperty("file.separator")
						+ fc.getSelectedFile().getName();
				if (DEBUG) {
	...
	public boolean writeFile(String fileName) throws Exception {
		...
		try {
			...
			if (optionslogbook.isEmpty()) {
				...
				fout.print("exerciseoptions|");
				// start i at 10 because the exercise options range in index
				// from
	 ...
	 * 
	 * @param evt
	 *            Name of action command triggered by the user
	 * 
	 * @return void
	 */
	public void actionPerformed(ActionEvent evt) {
		try {
			...
			if (logbook != null) {
				...
				if (source.equals("find") && evt.getSource() instanceof JButton) {
					find();
				} else if (source.equals("edit") && evt.getSource() instanceof JButton) {
				...
				else if (source.equals("cancel") && evt.getSource() instanceof JButton) {
					if (maintenancePanel.isVisible())
						cancelMaintRecord();
					else if (equipPanel.isVisible())
						cancelEquipPanel();
				...
				else if (source.equals("new") && evt.getSource() instanceof JButton) {
					if (maintenancePanel.isVisible())
						createMaintRecord();
					else {
						create();
				...
				else if (source.equals("save") && evt.getSource() instanceof JButton) {
					if (maintenancePanel.isVisible())
						saveMaintRecord();
					else if (equipPanel.isVisible())
						saveEquipPanel();
				...
				else if (source.equals("delete") && evt.getSource() instanceof JButton) {
					if (maintenancePanel.isVisible())
						deleteMaintRecord();
					else
						delete();
				...
				else if (source.equals("clear") && evt.getSource() instanceof JButton) {
					if (maintenancePanel.isVisible())
						clearMaintRecord();
					else if (equipPanel.isVisible())
						clearEquipPanel();
				...
				else if (source.equals("beginning") && evt.getSource() instanceof JButton) {
					getFirst();
				} else if (source.equals("previous") && evt.getSource() instanceof JButton) {
					if (maintenancePanel.isVisible())
						getPrevMaintRecord();
					else
						getPrev();
				...
				else if (source.equals("next") && evt.getSource() instanceof JButton) {
					if (maintenancePanel.isVisible())
						getNextMaintRecord();
					else
						getNext();
				...
				else if (source.equals("end") && evt.getSource() instanceof JButton) {
					getLast();
				}

				else if (source.equals("equiprun") && evt.getSource() instanceof JRadioButton) {
					...
					// make all textfields uneditable
					Component[] comp = { equipRunShoeSizeField, equipRunShoeModelField };
				...
				else if (source.equals("equipbike") && evt.getSource() instanceof JRadioButton) {
					equipSpecPanel.removeAll();
					GridBagConstraints gbc = new GridBagConstraints();
					gbc.anchor = GridBagConstraints.EAST;
					gbc.insets = new Insets(2, 2, 2, 2);
					...
					Component[] comp = { equipBikeNameField, equipBikeModelField, equipBikeSNField,
							equipBikeFrameSizeField, equipBikeTireSizeField, equipBikeShoeSizeField,
							equipBikeShoeModelField, equipBikeSeatPostDiameterField };// ~Component
																						// []
																						// comp
																						// =...
				...
				else if (source.equals("equipswim") && evt.getSource() instanceof JRadioButton) {
					equipSpecPanel.removeAll();
					GridBagConstraints gbc = new GridBagConstraints();
					gbc.anchor = GridBagConstraints.EAST;
					gbc.insets = new Insets(2, 2, 2, 2);
					...
					// make textfields uneditable
					Component[] comp = { equipSwimGoggleSizeField };
				...
				else if (source.equals("equipwalk") && evt.getSource() instanceof JRadioButton) {
					equipSpecPanel.removeAll();
					GridBagConstraints gbc = new GridBagConstraints();
					gbc.anchor = GridBagConstraints.EAST;
					gbc.insets = new Insets(2, 2, 2, 2);
					...
					// make textfields uneditable
					Component[] comp = { equipWalkShoeSizeField, equipWalkShoeModelField };
				...
				else if (source.equals("showgraph") && evt.getSource() instanceof JButton) {
					...
					try {
						...
						JOptionPane.showMessageDialog(this, "Please select an event before displaying the graph.",
								"Error", JOptionPane.ERROR_MESSAGE);
					}
				}
			} // ~if(logbook != null)...

			if (source.equals("openlog") && evt.getSource() instanceof JButton) {
			...
			else if (source.equals("open") && evt.getSource() instanceof JMenuItem) {
				...
				try {
					...
					if (fileExists && hasValidFirstRecord()) {
						...
						// enable
						Component[] compOn = { printItem, saveItem, saveAsItem, exitItem, backupItem, closeItem,
								removeallItem, optionsItem, findButton, newButton, deleteButton, editButton, saveButton,
								cancelButton, clearButton, beginButton, prevButton, nextButton, endButton };// ~Component
																											// []
																											// compOn
																											// =...
						...
						componentsEnabled(compOn, true);
						// disable
						Component[] compOff = { openItem, newItem };
			...
			else if (source.equals("new") && evt.getSource() instanceof JMenuItem) {
				repaint();
				createNewDatabase();
				resetForm();
				// d.print("creating new file");
				// enable
				Component[] compOn = { saveAsItem, closeItem, exitItem, newButton, saveButton, cancelButton,
						clearButton };// ~Component [] compOn =...

				componentsEnabled(compOn, true);
				// disable
				Component[] compOff = { openItem, newItem, printItem, saveItem, backupItem, removeallItem, optionsItem,
						findButton, deleteButton, editButton, beginButton, prevButton, nextButton, endButton };// ~Component
																												// []
																												// compOff
																												// =...
			...
			/*
			 * else if (source.equals("print") && evt.getSource() instanceof
			 * JMenuItem) { repaint(); printWorkout((Workout)logbook.elementAt
			 * (visibleWorkoutRecord - 1),visibleWorkoutRecord); }
			 */
			else if (source.equals("save") && evt.getSource() instanceof JMenuItem) {
				repaint();
			...
			else if (source.equals("saveas") && evt.getSource() instanceof JMenuItem) {
				repaint();
			...
			else if (source.equals("backup") && (evt.getSource() instanceof JMenuItem)) {
				repaint();
				...
				try {
					...
					String filePath = properties.getProperty("user.dir") + properties.getProperty("file.separator")
							+ "backup.dat";
					...
					if (fileSaved) {
						...
						componentsEnabled(compOff, false);
						JOptionPane.showMessageDialog(this, "Backup written to " + filePath, "Information",
								JOptionPane.INFORMATION_MESSAGE);
					} // ~if(fileSaved)...
				} // ~try...

				catch (Exception e) {
					JOptionPane.showMessageDialog(this, "Error while making backup. Reason unknown.", "Error",
							JOptionPane.ERROR_MESSAGE);
				}
			} // ~(evt.getSource() instanc...

			else if (source.equals("close") && (evt.getSource() instanceof JMenuItem)) {
				repaint();
				...
				if (!identical(origlogbook, logbook) || !identical(origmntlogbook, mntlogbook)
						|| !identical(origequiplogbook, equiplogbook) || !identical(origoptionslogbook, optionslogbook)
								&& !origlogbook.isEmpty() && !logbook.isEmpty()) {
					int option = JOptionPane.showConfirmDialog(this, "Do you want to save your latest changes?",
							"Save Latest Changes?", JOptionPane.YES_NO_OPTION);
					if (option == JOptionPane.YES_OPTION) {
			...
			else if (source.equals("exit") && (evt.getSource() instanceof JMenuItem)) {
				repaint();
			...
			else if (source.equals("remove all") && (evt.getSource() instanceof JMenuItem)) {
				repaint();
				int answer = JOptionPane.showConfirmDialog(this, "This action will delete all records!", "Confirmation",
			...
			else if (source.equals("options") && (evt.getSource() instanceof JMenuItem)) {
				showOptions(this);
			}

			else if ((source.equals("run")) && (evt.getSource() instanceof JRadioButton) && statsPanel.isVisible()) {
			...
			else if ((source.equals("bike")) && (evt.getSource() instanceof JRadioButton) && statsPanel.isVisible()) {
			...
			else if ((source.equals("swim")) && (evt.getSource() instanceof JRadioButton) && statsPanel.isVisible()) {
			...
			else if ((source.equals("calcmilescheck") || source.equals("calckiloscheck"))
					&& (evt.getSource() instanceof JCheckBox) && calcPanel.isVisible()) {
				calcResetFields();
			...
			// Miles checked
			// time checked
			else if (source.equals("calctimeradio") && (evt.getSource() instanceof JRadioButton)
			...
			// miles checked
			// distance checked
			else if (source.equals("calcdistanceradio") && (evt.getSource() instanceof JRadioButton)
					&& calcMilesCheck.isSelected()) {
				calcResetFields();
				calcDataLabel.setEnabled(true);
				calcPaceMphLabel.setEnabled(true);
			...
			// miles checked
			// pace checked
			else if (source.equals("calcpaceradio") && (evt.getSource() instanceof JRadioButton)
					&& calcMilesCheck.isSelected()) {
				calcResetFields();
				calcDataLabel.setEnabled(true);
				calcTimeLabel.setEnabled(true);
			...
			// pace mph checked
			else if (source.equals("calcpacemphradio") && (evt.getSource() instanceof JRadioButton)
			...
			else if (source.equals("calcpaceminpermileradio") && (evt.getSource() instanceof JRadioButton)
					&& calcPaceMinPerMileRadio.isSelected()) {
				calcPaceMphField.setEnabled(false);
			...
			// pace kph
			else if (source.equals("calcpacekphradio") && (evt.getSource() instanceof JRadioButton)
			...
			// pace min per km
			else if (source.equals("calcpaceminperkmradio") && (evt.getSource() instanceof JRadioButton)
			...
			// pace min per 100m
			else if (source.equals("calcpaceminsper100mradio") && (evt.getSource() instanceof JRadioButton)
					&& calcPaceMinsPer100mRadio.isSelected()) {
				calcPaceKphField.setEnabled(false);
				calcPaceMinPerKmField.setEnabled(false);
			...
			// pace min per 100yds
			else if (source.equals("calcpaceminsper100ydsradio") && (evt.getSource() instanceof JRadioButton)
					&& calcPaceMinsPer100ydsRadio.isSelected()) {
				calcPaceMphField.setEnabled(false);
				calcPaceMinPerMileField.setEnabled(false);
			...
			else if (source.equals("calcbutton") && evt.getSource() instanceof JButton && calcPanel.isVisible()) {
				calcFields();
			}

			else if (source.equals("calcresetbutton") && evt.getSource() instanceof JButton && calcPanel.isVisible()) {
				calcResetFields();
				calcSelectionLabel.setEnabled(true);
	 ...
	 * 
	 * @param evt
	 *            Name of tab selected by the user
	 * 
	 * @return void
	 */
	public void stateChanged(ChangeEvent evt) {
		...
		if (n == 0 && logbook != null && logbooksize > 0) {
			...
			// enable
			Component[] compOn = { findButton, newButton, deleteButton, editButton, saveButton, cancelButton,
					clearButton, beginButton, prevButton, nextButton, endButton };// ~Component
																					// []
																					// compOn
																					// =...
			...
			statusField.setText("Workout Record " + visibleWorkoutRecord + " of " + logbooksize);
		} // ~if(n == 0 && logbook != ...
		...
		// Maintenance tab
			...
			// enable
			Component[] compOn = { newButton, deleteButton, editButton, saveButton, cancelButton, clearButton,
					prevButton, nextButton };// ~Component [] compOn =...

			componentsEnabled(compOn, true);
			// disable
			Component[] compOff = { findButton, beginButton, endButton };
		...
		else if (n == 4) // Equipment Tab
		{
			...
			// enable
			Component[] compOn = { editButton, cancelButton, saveButton, clearButton };

			componentsEnabled(compOn, true);
			// disable
			Component[] compOff = { findButton, newButton, deleteButton, beginButton, prevButton, nextButton,
					endButton, };// ~Component [] compOff =...
	...
	private JMenu databaseMenu, editMenu, optionsMenu, helpMenu;
	private JMenuItem openItem, newItem, printItem, saveItem, saveAsItem, exitItem, backupItem, closeItem,
			removeallItem, optionsItem, howToItem, aboutItem;
	private JButton findButton, editButton, cancelButton, newButton, saveButton, deleteButton, clearButton, beginButton,
			prevButton, nextButton, endButton, calcButton, calcResetButton, maintOpenButton, statsGraphButton;
	private JLabel dateLabel, runLabel, runPaceLabel, runDistanceMeasureLabel, bikeLabel, bikePaceLabel,
			bikeDistanceMeasureLabel, swimLabel, swimPaceLabel, swimDistanceMeasureLabel, paceLabel, eventLabel,
			distanceLabel, timeLabel, avgSpeedLabel, routeLabel, weatherLabel, tempLabel, pulseLabel, summaryLabel,
			weightLabel, puLabel, suLabel, totalMilesToDateLabel, avgMilesLabel, maintDateLabel, maintWorkLabel,
			statsEventLabel, statsAvgSpeedLabel, statsAvgPaceLabel, statsLeftAxisOptionLabel, statsRightAxisOptionLabel,
			calcSelectionLabel, calcDistanceMilesLabel, calcDistanceKiloLabel, calcTimeLabel, calcTimeConvertedLabel,
			calcPaceMphLabel, calcPaceKphLabel, calcResultLabel, calcDataLabel, calcMinPerMileLabel, calcMinPerKmLabel,
			calcMinPer100mLabel, calcMinPer100ydsLabel, equipSelectOption, equipRunShoeSizeLabel,
			equipRunShoeModelLabel, equipBikeNameLabel, equipBikeModelLabel, equipBikeSNLabel, equipBikeFrameSizeLabel,
			equipBikeTireSizeLabel, equipBikeShoeSizeLabel, equipBikeShoeModelLabel, equipBikeSeatStemDiameterLabel,
			equipSwimGoggleSizeLabel, equipWalkShoeSizeLabel, equipWalkShoeModelLabel;
	private JTextField monthField, dayField, yearField, statusField, puField, suField, totalMilesToDateField,
			avgMilesField, statsAvgSpeedField, maintDateField, calcDistanceMilesField, calcDistanceKiloField,
			calcTimeField, calcTimeConvertedField, calcPaceMphField, calcPaceKphField, calcPaceMinPerMileField,
			calcPaceMinPerKmField, calcPaceMinPer100mField, calcPaceMinPer100ydsField, equipRunShoeSizeField,
			equipRunShoeModelField, equipBikeNameField, equipBikeModelField, equipBikeSNField, equipBikeFrameSizeField,
			equipBikeTireSizeField, equipBikeShoeSizeField, equipBikeShoeModelField, equipBikeSeatPostDiameterField,
			equipSwimGoggleSizeField, equipWalkShoeSizeField, equipWalkShoeModelField, todaysDateField;
	private JTextArea summaryTextArea, maintWorkTextArea;
	private ButtonGroup eventGroup, weatherGroup, statsEventGroup, calcResultGroup, calcSelectionGroup, equipEventGroup,
			calcPaceMileRadioGroup, calcPaceKmRadioGroup;
	private JRadioButton statsBikeRadio, statsRunRadio, statsSwimRadio, calcTimeRadio, calcDistanceRadio, calcPaceRadio,
			equipRunRadio, equipBikeRadio, equipSwimRadio, equipWalkRadio, calcPaceMphRadio, calcPaceMinPerMileRadio,
			calcPaceKphRadio, calcPaceMinPerKmRadio, calcPaceMinsPer100ydsRadio, calcPaceMinsPer100mRadio;
	private JCheckBox kiloOnCheckBox, calcMilesCheck, calcKilosCheck;
	private JComboBox tempComboBox, weatherComboBox, pulseComboBox, weightComboBox, statsLeftAxisOptionComboBox,
			statsRightAxisOptionComboBox;
	private JScrollPane scrollPane;
	private JPanel actionPanel, tabPanel, mainPanel, dateTempPanel, summaryPanel, stationaryPanel, statsPanel,
			statsFieldPanel, statsGraphPanel, eventPanel, weatherPanel, bottomPanel, navPanel, datePanel, statusPanel,
			maintenancePanel, maintButtonPanel, statsEventPanel, calcPanel, calcButtonPanel, equipPanel,
			equipEventPanel, equipSpecPanel, equipOuterEventPanel, equipOuterSpecPanel;
	...
	private static Vector logbook, origlogbook, mntlogbook, origmntlogbook, equiplogbook, origequiplogbook,
			optionslogbook, origoptionslogbook;
	private Workout w;
