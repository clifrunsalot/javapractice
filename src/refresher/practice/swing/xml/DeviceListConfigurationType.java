...
public class DeviceListConfigurationType {
	...
	/**
	 * These string variables represent node names of interest when the JTree is
	 * passed in.
	 */
	 ...
	 */
	private final String[] nodeNameArray = { ID, TYPENAME, DEVICELIST, DEVICENAME };
	...
	/**
	 * This represents the Domain Name common to all devices on the target.
	 */
	private String domainName;

	/**
	 * This represents the IOR String common to all devices on the target.
	 */
	...
	/**
	 * This returns TRUE if the string passed in is in the array of valid node
	 * names.
	 */
	...
	private DeviceType getDeviceType(Object n) {
		...
		/**
		 * For each child of the node passed in, determine its type and extract
		 * its value.
		 */
		for (int i = 0; i < count; i++) {
			...
			/**
			 * Is this an ELEMENT-TYPE node with a name in the nodeNameArray and
			 * does it have a non-empty node value?
			 */
			if (isElementNode(node) && isValidNodeName(node.getName())) {
	...
	/**
	 * This extracts the configuration information from the JTree object passed
	 * in
	 */
	private void processTree(JTree tree) {
		...
		for (int i = 0; i < count; i++) {
			...
			/**
			 * If the node name is "IORStringPath", extract and store the
			 * string.
			 */
