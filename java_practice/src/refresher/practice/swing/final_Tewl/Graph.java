...
public class Graph extends JFrame implements ActionListener, WindowListener {
	public Graph(Vector log, String event, Component logParent, String leftAxis, String rightAxis) {
		d = new Debug();
	...
	public String getShortAxisName(String event, String axisOption) {
		...
		if (axisOption.equals("Distance in Miles") && (event.equals("run") || event.equals("bike")))
			shortAxisName = "Miles";
		else if (axisOption.equals("Distance in Yards") && event.equals("swim"))
			shortAxisName = "Yards";
		else if (axisOption.equals("Pace (mph)") && (event.equals("run") || event.equals("bike")))
			shortAxisName = "Mph";
		else if (axisOption.equals("Pace (mins/100 yds)") && event.equals("swim"))
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (source.equals("display") & evt.getSource() instanceof JButton) {
			...
			if (isValid(recordStr)) {
				...
				if (record > 0 && record <= eventList.size()) {
					Workout w = new Workout();
					w = find(record);
					if (recordDialog.getName().equals("recordDialog") && !recordDialog.isVisible()) {
					...
					JOptionPane.showMessageDialog(this, "Invalid Record!\n" + "That record does not exist.", "Error",
							JOptionPane.ERROR_MESSAGE);
				}
			} // ~if (isValid(recordStr))...

			else {
				JOptionPane.showMessageDialog(this, "Invalid Record!\n" + "Record must be an integer.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		}
		repaint();
	...
	private Debug d;
	private JScrollPane scrollPaneGraph, scrollPaneYPanel, scrollPaneZPanel;
	private JPanel buttonPanel, innerPanel, rightPanel, innerAndRightPanel, mainPanel;
	private JSplitPane splitPane1Vertical, splitPane2Vertical;
	private JButton displayButton;
	private JTextField displayField;
	private JComboBox zOptionComboBox;
	private JLabel zOptionLabel;
	private int record;
	private String recordStr, localEvent, localLeftAxis, localRightAxis;
	private RecordDialog recordDialog;
	private Vector localLog, eventList;
