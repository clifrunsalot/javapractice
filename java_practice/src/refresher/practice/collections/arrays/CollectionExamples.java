...
public class CollectionExamples {
    ...
    public static void main(String[] args) {
        ...
        LogIt logger = new LogIt(CollectionExamples.class.getName());

        CollectionExamples.isPalindrome("a,b|}c{b+a");

        logger.logFunc("isPalindrome", "\"a,b|}c{b+a\"", String.valueOf(isPalindrome("a,b|}c{b+a")));
        logger.logFunc("isPalindrome", "\"Anne. I vote more cars race Rome to Vienna!\"",
                String.valueOf(isPalindrome("Anne. I vote more cars race Rome to Vienna!")));
        logger.logFunc("isPalindrome", "\"A but tuxa\"", String.valueOf(isPalindrome("A but tuxa")));

        logger.logFunc("convertStringIntoSortedList", "\"hello, world. this is a simple string.\"",
                convertStringIntoSortedList("hello, world. this is a simple string."));

        int[] ary1 = {9, 8, 7, 6, 7, 8, 3, 2, 1, 5, -3};
        logger.logFunc("convertNumbersIntoSortedList", "" + LogIt.toIntString(ary1), convertNumbersIntoSortedList(ary1));

        int[] ary2 = {99, 45, -6, 33, 24, 2, 2, 567, 2, 99, -6};
        logger.logFunc("useMapToFindDuplicateNumbers", LogIt.toIntString(ary2), useMapToFindDuplicateNumbers(ary2));

        String str = "Sam I am and am I Sam";
        logger.logFunc("useMapToFindDuplicateWords", "\"" + str + "\"", useMapToFindDuplicateWords(str));

        String parens = "[[[]]]";
        char leftParen = '[';
        char rightParen = ']';
        logger.logFunc("hasMatchingBrackets", "(\"" + parens + "\",'" + leftParen + "','" + rightParen + "')",
        ...
        for (int i = 1; i < 10000000; i++) {
            if (isFactorial(i))
                logger.logFunc("isFactorial", "" + i, String.valueOf(isFactorial(i)));
        }
    }
